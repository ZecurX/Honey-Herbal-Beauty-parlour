                        ...
                        {loading ? (
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-5">
                                {filteredGallery.map((item, index) => (
                                    <div
                                        key={item.id}
                                        className="group relative rounded-2xl overflow-hidden cursor-pointer shadow-lg transform hover:-translate-y-2 transition-all duration-500"
                                        style={{ transitionDelay: `${index * 50}ms` }}
                                    ...
                                    >
                                        ...
                                        <div className="absolute inset-0 bg-gradient-to-t from-charcoal/90 via-charcoal/20 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-400">
                                            <div className="absolute bottom-0 left-0 right-0 p-3 md:p-5 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-400">
                                                <span className="text-secondary text-xs font-medium inline-block mb-1 md:mb-2">
                                                    {item.category}
                                                </span>
                                                <p className="text-white font-medium text-sm md:text-base line-clamp-2">{item.caption}</p>
                                            </div>
                                        </div>
                                        {/* Hover Icon */}
                                        <div className="absolute top-2 right-2 md:top-4 md:right-4 w-8 h-8 md:w-10 md:h-10 rounded-full glass-card flex items-center justify-center opacity-0 group-hover:opacity-100 scale-50 group-hover:scale-100 transition-all duration-400">
                                            <svg className="w-4 h-4 md:w-5 md:h-5 text-charcoal" fill="none" stroke="currentColor" viewBox="0 0 24 24">
