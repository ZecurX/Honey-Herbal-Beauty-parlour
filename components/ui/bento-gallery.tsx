...
const ImageModal = ({
    ...
    return (
        ...
        >
            ...
            <button
                onClick={onClose}
                className="absolute right-6 top-6 w-12 h-12 rounded-full bg-white/10 flex items-center justify-center text-white/80 transition-colors hover:text-white hover:bg-white/20"
...
const InteractiveImageBentoGallery: React.FC<InteractiveImageBentoGalleryProps> = ({
    ...
    return (
        <section
            ref={targetRef}
            className={cn("relative w-full overflow-hidden py-16 sm:py-24", className)}
        >
            <motion.div
                style={{ opacity, y }}
                className="container mx-auto px-4 text-center"
            >
                <span className="text-charcoal text-sm font-medium tracking-widest uppercase mb-4 inline-block">
                    Our Work
                </span>
                ...
                <p className="mx-auto max-w-2xl text-lg text-gray-light">
                    {description}
                </p>
            </motion.div>
